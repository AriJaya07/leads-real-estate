...
import type { actorTemplateInput } from "@/application/collection/actor-template-input.schema";

/**
 * Shared "which Apify actor scrapes what" registry UI — the table + the
 * register form, reused by both the tenant-admin entry point
 * (`ActorTemplateManager`, `/admin/collection`) and the Super Admin one
 * (`SourceRegistryManager`, `/platform/sources`). They write the same
 * `actor_templates` table through two separately audited server actions
 * (`application/collection/actor-templates.actions.ts` vs
 * `application/platform/sources.actions.ts`) — this module only owns the
 * form/table markup, never which action gets called.
 */

export type ActorTemplateFormValues = z.infer<typeof actorTemplateInput>;
type SubmitResult<T> = { data?: T; serverError?: string; validationErrors?: unknown } | undefined;
export type SubmitActorTemplate = (input: ActorTemplateFormValues) => Promise<SubmitResult<{ id: string }>>;
export type SetActorTemplateEnabled = (input: { id: string; enabled: boolean }) => Promise<SubmitResult<{ ok: boolean }>>;
...
export function ActorTemplateTable({
  templates,
  categoryById,
  toggleEnabled,
  usageByTemplate,
  ...
  toggleEnabled: SetActorTemplateEnabled;
  /** Cross-tenant request counts, Super Admin view only — omit to hide the column entirely (tenant admins only see their own company's usage, elsewhere). */
  ...
  const toggle = useCallback(
    async (template: ActorTemplateRow) => {
      await run(template.id, () => toggleEnabled({ id: template.id, enabled: !template.enabled }), {
  ...
  return (
    <DataTable minWidth="min-w-[760px]">
      <DataTableHead>
        ...
        <th>Actor id</th>
        {usageByTemplate && <th className="w-24">Requests</th>}
      ...
      <tbody>
        {templates.map((template) => (
          <TemplateRow
            ...
            toggle={toggle}
            usageCount={usageByTemplate?.get(template.id)}
            showUsage={usageByTemplate !== undefined}
          />
        ))}
      </tbody>
    </DataTable>
 ...
 * re-renders `ActorTemplateTable`, but only the busy row's own `busy` prop
...
const TemplateRow = memo(function TemplateRow({
  ...
  return (
    <tr className="border-border border-t align-middle">
      ...
      {showUsage && (
        <td className="text-muted-foreground px-3 py-2.5 font-mono text-xs tabular-nums">{usageCount ?? 0}</td>
      )}
      <td className="px-3 py-2.5">
...
export function ActorTemplateForm({
  categories,
  submitAction,
  onDone,
}: {
  categories: CategoryOption[];
  submitAction: SubmitActorTemplate;
  onDone: () => void;
}) {
  const router = useRouter();
  const { busyId, run } = useServerAction();
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const cleanedParamSchema = paramSchema.filter((f) => f.key.trim() && f.label.trim()).map(toActorParamField);
