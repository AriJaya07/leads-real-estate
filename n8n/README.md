...
4. `workflows/08-notification-delivery.json` and
   ...
   port, a new `AVERONAI_NOTIFY_SHARED_SECRET` env var, and
...
- **Environment variables** (n8n instance-level, Settings → Environment or your
  ...
  - `AVERONAI_NOTIFY_SHARED_SECRET` — only for workflows 08/09 (not yet wired
    up); would need to match a new app-side env var of the same name.
  - `AVERONAI_NOTIFY_FROM_EMAIL` — only for workflows 08/09 (not yet wired up).
